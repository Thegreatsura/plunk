...
| `snoozedUntil` | Timestamp when a snooze ends, or `null`. See [Snoozing](#snoozing) |
...
- **Self-service, temporarily** by snoozing instead of unsubscribing — see below.
...
A recipient can pause emails for 2 weeks, 1 month, 6 months, or a year instead of leaving permanently. A snoozed contact has `subscribed: false` — so every rule on this page applies to them unchanged — plus a `snoozedUntil` timestamp saying when they come back.

A background job resubscribes them when the window ends and fires `contact.subscribed` with `reason: "snooze_expired"`. The snooze itself fires `contact.unsubscribed` with `reason: "snooze"`.

Any other change to `subscribed` cancels a running snooze, including a bounce or complaint — a suppressed address is never resurrected by a snooze that was in flight.

See [Unsubscribe & preferences pages](/guides/unsubscribe-pages#snoozing) for the full behaviour.
